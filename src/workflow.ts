...
import type {
  ...
  OntologyActionHandlers,
  OntologyFunctionHandlers,
  OntologyObject,
  OntologyObjectId,
...
type ActionInput = Readonly<Record<string, OntologyPropertyValue>>
 ...
 */
export function createBidActionHandlers(startWorkflow: StartWorkflow): OntologyActionHandlers {
  const handlers: Record<string, OntologyActionHandler> = {
 ...
 */
export function createBidFunctionHandlers(): OntologyFunctionHandlers {
  return {
    [BID_FUNCTIONS.requirementsForProject]: (input, ontology) => Promise.resolve(
      objectsForProject(ontology, BID_OBJECT_TYPES.requirement, requiredString(input, 'projectId')),
    ),
    [BID_FUNCTIONS.evidenceCoverage]: (input, ontology) => {
      ...
      const matchedRequirementIds = new Set(
        objectsForProject(ontology, BID_OBJECT_TYPES.evidenceMatch, projectId)
          .map(match => stringProperty(match, 'requirementId')),
      )
      return Promise.resolve(matchedRequirementIds.size / requirements.length)
    },
    [BID_FUNCTIONS.unresolvedRiskCount]: (input, ontology) => Promise.resolve(
      objectsForProject(ontology, BID_OBJECT_TYPES.risk, requiredString(input, 'projectId'))
        .filter(risk => !booleanProperty(risk, 'resolved')).length,
    ),
  }
}

const createProject: OntologyActionHandler = async (input, ontology) => {
  const name = requiredString(input, 'name')
  return await ontology.createObject({ type: BID_OBJECT_TYPES.project, properties: { name, stage: '资料准备' } })
}

const importTender: OntologyActionHandler = async (input, ontology) => {
...
const analyzeTender: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  ...
  if (document === undefined) throw new Error(`bid-studio: project '${project.id}' has no tender document`)
  const lines = tenderLines(stringProperty(document, 'content'))
  let first: OntologyObject | undefined
  ...
  if (first === undefined) throw new Error('bid-studio: tender document contains no analyzable requirement lines')
  await ontology.updateObject({ id: document.id, properties: { status: '已解析' } })
  return first
}

const seedEvidence: OntologyActionHandler = async (_input, ontology) => {
  const existing = ontology.listObjects(BID_OBJECT_TYPES.evidence)
  let first = existing[0]
  for (const record of EVIDENCE_CATALOG) {
    if (existing.some(candidate => stringProperty(candidate, 'title') === record.title)) continue
    const created = await ontology.createObject({ type: BID_OBJECT_TYPES.evidence, properties: record })
    first ??= created
  }
  if (first === undefined) throw new Error('bid-studio: evidence catalog did not produce any record')
  return first
}

const matchEvidence: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
...
const assessRisks: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  const requirements = objectsForProject(ontology, BID_OBJECT_TYPES.requirement, project.id)
  ...
  let first = risks[0]
  for (const requirement of requirements) {
    if (matches.some(candidate => stringProperty(candidate, 'requirementId') === requirement.id)) continue
...
const generateResponses: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  const requirements = objectsForProject(ontology, BID_OBJECT_TYPES.requirement, project.id)
  const matches = objectsForProject(ontology, BID_OBJECT_TYPES.evidenceMatch, project.id)
  const evidence = ontology.listObjects(BID_OBJECT_TYPES.evidence)
  ...
  for (const requirement of requirements) {
    ...
    const evidenceTitles = requirementMatches.map(match => evidence.find(candidate => candidate.id === stringProperty(match, 'evidenceId')))
      .filter((candidate): candidate is OntologyObject => candidate !== undefined)
  ...
  if (first === undefined) throw new Error(`bid-studio: project '${project.id}' has no generated responses`)
  await ontology.updateObject({ id: project.id, properties: { stage: '应答生成' } })
  return first
}

const submitReview: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  ...
  if (responses.length === 0) throw new Error(`bid-studio: project '${project.id}' has no responses to review`)
...
const approveReview: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  const reviewer = requiredString(input, 'reviewer')
  const review = objectsForProject(ontology, BID_OBJECT_TYPES.review, project.id)[0]
  if (review === undefined) throw new Error(`bid-studio: project '${project.id}' has no pending review`)
...
const exportDocument: OntologyActionHandler = async (input, ontology) => {
  const project = requireProject(input, ontology)
  const review = objectsForProject(ontology, BID_OBJECT_TYPES.review, project.id)[0]
  ...
  if (existing === undefined) await ontology.createLink({ type: BID_LINK_TYPES.projectArtifact, from: project.id, to: artifact.id })
...
async function runToReview(
  input: ActionInput,
  ontology: OntologyActionContext,
  handlers: Record<string, OntologyActionHandler>,
    ...
    const workflow = startWorkflow({
      ...
      activities: {
        'bid.execute-step': async (activityInput, signal) => {
          ...
          if (step === undefined) throw new Error(`bid-studio: unknown automated workflow step '${stepId}'`)
          ...
          if (handler === undefined) throw new Error(`bid-studio: workflow step '${step.id}' has no adapter`)
        ...
        'bid.submit-review': async (_activityInput, signal) => {
          signal.throwIfAborted()
          const review = await submitReview({ projectId: project.id, runId: run.id }, ontology) as OntologyObject
...
function activityString(input: unknown, key: string): string {
...
function requireProject(input: ActionInput, ontology: OntologyActionContext): OntologyObject {
  const projectId = requiredString(input, 'projectId')
  const project = ontology.listObjects(BID_OBJECT_TYPES.project).find(candidate => candidate.id === projectId)
  if (project === undefined) throw new Error(`bid-studio: project not found: ${projectId}`)
  return project
}

function requireRun(input: ActionInput, ontology: OntologyActionContext): OntologyObject {
  const runId = requiredString(input, 'runId')
  const run = ontology.listObjects(BID_OBJECT_TYPES.workflowRun).find(candidate => candidate.id === runId)
  if (run === undefined) throw new Error(`bid-studio: workflow run not found: ${runId}`)
  return run
}

function requiredString(input: ActionInput, key: string): string {
  const value = input[key]
  if (typeof value !== 'string' || value.trim().length === 0) throw new Error(`bid-studio: ${key} must be a non-empty string`)
  return value.trim()
}

function optionalString(input: ActionInput, key: string): string | undefined {
  const value = input[key]
...
function booleanProperty(object: OntologyObject, key: string): boolean {
  return object.properties[key] === true
}

function objectsForProject(
  ontology: OntologyReadContext,
...
function tenderLines(content: string): readonly { number: number; text: string }[] {
...
function scoreFrom(text: string): number | undefined {
  const match = /(\d+(?:\.\d+)?)\s*分/u.exec(text)
  return match?.[1] === undefined ? undefined : Number(match[1])
}

function classifyRequirement(text: string): '资格' | '商务' | '技术' | '交付' {
...
function selectEvidence(
  requirement: OntologyObject,
  evidence: readonly OntologyObject[],
): { evidence: OntologyObject; confidence: number; rationale: string } | undefined {
  ...
  if (keyword !== undefined) return { evidence: keyword, confidence: 0.94, rationale: '要求文本命中证据关键词。' }
...
function responseContent(requirement: OntologyObject, evidenceTitles: readonly string[], risk: OntologyObject | undefined): string {
  const title = stringProperty(requirement, 'title')
...
function exportMarkdown(project: OntologyObject, ontology: OntologyActionContext): string {
  const requirements = objectsForProject(ontology, BID_OBJECT_TYPES.requirement, project.id)
  const responses = objectsForProject(ontology, BID_OBJECT_TYPES.response, project.id)
  const risks = objectsForProject(ontology, BID_OBJECT_TYPES.risk, project.id)
...
async function updateStep(
  ...
  if (step !== undefined) await ontology.updateObject({ id: step.id, properties: { status, detail } })
}

async function stepResultDetail(
  step: BidWorkflowStepDefinition,
  projectId: OntologyObjectId,
  ontology: OntologyActionContext,
): Promise<string> {
